...
{
    /// <summary>
    /// Handler за печат на ElTest баркод
    /// </summary>
    public class PrintElTestBarcodeCommandHandler : IRequestHandler<PrintElTestBarcodeCommand, TorkResultDto>
    {
        private readonly UretimOtomasyonContext _context;
        private readonly IMapper _mapper;

        public PrintElTestBarcodeCommandHandler(UretimOtomasyonContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }

        public async Task<TorkResultDto> Handle(PrintElTestBarcodeCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var result = torkService.GitElTestBarcodeBas(barcode);
