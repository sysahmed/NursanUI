...
{
    /// <summary>
    /// Handler за добавяне на Paketleme ID
    /// </summary>
    public class AddPaketlemeIdCommandHandler : IRequestHandler<AddPaketlemeIdCommand, TorkResultDto>
    {
        private readonly UretimOtomasyonContext _context;
        private readonly IMapper _mapper;

        public AddPaketlemeIdCommandHandler(UretimOtomasyonContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }

        public async Task<TorkResultDto> Handle(AddPaketlemeIdCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var donanimCount = _mapper.Map<IzDonanimCount>(request.DonanimCount);
                
                var result = torkService.AddDPaketlemeId(donanimCount);
