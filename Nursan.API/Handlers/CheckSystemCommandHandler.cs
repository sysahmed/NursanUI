...
{
    /// <summary>
    /// Handler за проверка на системата
    /// </summary>
    public class CheckSystemCommandHandler : IRequestHandler<CheckSystemCommand, TorkResultDto>
    {
        private readonly UretimOtomasyonContext _context;
        private readonly IMapper _mapper;

        public CheckSystemCommandHandler(UretimOtomasyonContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }

        public async Task<TorkResultDto> Handle(CheckSystemCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var result = torkService.GitSytemeSayiBac(barcode);
