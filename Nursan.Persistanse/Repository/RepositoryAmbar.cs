...
{
    public class RepositoryAmbar<IEntity> : IRepositoryAmabar<IEntity> where IEntity : class
    {
        protected readonly DbContext _context;
        private readonly DbSet<IEntity> _dbSet;
        private static string msage;
        public RepositoryAmbar(DbContext context)
        {
            _context = context;
            _dbSet = _context.Set<IEntity>();
        }
        IDataResult<DateTime> IRepositoryAmabar<IEntity>.TarihGetir()
        {
            try
            {
        ...
        /// <returns></returns>
        IResults IRepositoryAmabar<IEntity>.Delete(IEntity entity)
        {
            try
            {
        ...
        /// <returns></returns>
        IDataResult<List<IEntity>> IRepositoryAmabar<IEntity>.GetAll(Expression<Func<IEntity, bool>> predicate)
        {
            try
            {
                using (var context = new AmbarContext())
                {
                    var result = predicate == null
                        ...
                        : context.Set<IEntity>().Where(predicate).ToList();
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        IDataResult<IEntity> IRepositoryAmabar<IEntity>.GetById(int id)
        {
            try
            {
        ...
        /// <param name="predicate"></param>
        /// <returns></returns>
        IDataResult<IEntity> IRepositoryAmabar<IEntity>.Get(Expression<Func<IEntity, bool>> predicate)
        {
            try
            {
                using (var context = new AmbarContext())
                {
                    ...
                    var result = context.Set<IEntity>().FirstOrDefault(predicate);
        ...
        IDataResult<IEntity> IRepositoryAmabar<IEntity>.Add(IEntity entity)
        {
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        IDataResult<IEntity> IRepositoryAmabar<IEntity>.Update(IEntity entity)
        {
            try
            {
                using (var context = new AmbarContext())
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        IResults IRepositoryAmabar<IEntity>.Delete(int id)
        {
            try
            {
